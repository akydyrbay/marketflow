...
type Cache interface {
	...
	StoreLatestPrice(ctx context.Context, update PriceUpdate) error
}

// postgres
type PriceRepository interface {
	// StoreStats(stat PriceStats) error
	StoreStatsBatch(ctx context.Context, stats []PriceStats) error
	StorePriceUpdate(ctx context.Context, update PriceUpdate) error
